...
class ProfileVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 로그인 상태에 따라 적절히 로그인/로그아웃 버튼을 출력한다.
        self.drawBtn()
        
        // 프로필 이미지 탭시 이미지 수정
        let tap = UITapGestureRecognizer(target: self, action: #selector(profile(_:)))
        self.profileImage.addGestureRecognizer(tap) // 객체 등록
        self.profileImage.isUserInteractionEnabled = true // 해당 객체가 사용자와 상호반응 할 수 있도록 허락하는 역할
    ...
    // 이미지를 선택하면 이 메소드가 자동으로 호출된다 -> 이미지 정보를 전달하는 역할
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let img = info[UIImagePickerController.InfoKey.editedImage] as? UIImage {
            self.uinfo.profile = img
            self.profileImage.image = img
        }
        // 이 구문 누락시 이미지 피커 컨트롤러 창은 닫히지 않는다
